...
async function approve() {
  ...
  const tx = await contract.approve(pancakeRouter.address, valueToApprove, {
...
async function setInitialStopLoss(token) {
  ...
  token.stopLoss = ethers.utils.parseUnits(
    (
      parseFloat(ethers.utils.formatUnits(token.intitialValue)) -
      parseFloat(ethers.utils.formatUnits(token.intitialValue)) *
        (token.stopLossPercent / 100)
    )
      .toFixed(8)
      .toString()
  );
...
async function setNewStopLoss(token) {
  ...
  token.stopLoss = ethers.utils.parseUnits(
    (
      parseFloat(ethers.utils.formatUnits(token.currentValue)) -
      parseFloat(ethers.utils.formatUnits(token.currentValue)) *
        (token.stopLossPercent / 100)
    )
      .toFixed(8)
  ...
  try {
    ...
    token.contract.on("Transfer", async (from, to, value, event) => {
      ...
      if (!currentValue.eq(ethers.constants.Zero)) {
        ...
        let currentValueString = parseFloat(
        ...
        const takeProfit = (
          (parseFloat(ethers.utils.formatUnits(token.intitialValue)) *
            (token.profitPercent + token.tokenSellTax)) /
            100 +
          parseFloat(ethers.utils.formatUnits(token.intitialValue))
        )
          .toFixed(8)
        ...
        let targetValueToSetNewStopLoss = ethers.utils.parseUnits(
          (
            parseFloat(ethers.utils.formatUnits(token.newValue)) *
              (token.trailingStopLossPercent / 100) +
            parseFloat(ethers.utils.formatUnits(token.newValue))
          )
            .toFixed(8)
            .toString()
        ...
        if (
          currentValue.gt(targetValueToSetNewStopLoss) &&
          token.trailingStopLossPercent > 0
        ) {
        ...
        try {
          if (token.previousValue.gt(token.currentValue)) {
            console.log(
              `-- ${tokenName} -- Current Value BNB: ${
                "\u001b[1;31m" + currentValueString + "\u001b[0m"
              } -- Profit At: ${ethers.utils.formatUnits(
                profitDesired
              )} -- Stop Loss At: ${ethers.utils.formatUnits(
                token.stopLoss
              )} -- New Stop loss At: ${ethers.utils.formatUnits(
                targetValueToSetNewStopLoss
              )}`
            );
          } else {
            console.log(
              `-- ${tokenName} -- Current Value BNB: ${
                "\u001b[1;32m" + currentValueString + "\u001b[0m"
              } -- Profit At: ${ethers.utils.formatUnits(
                profitDesired
              )} -- Stop Loss At: ${ethers.utils.formatUnits(
                token.stopLoss
        ...
        if (currentValue.gte(profitDesired)) {
          if (
            buyCount <= config.numberOfTokensToBuy &&
            token.didBuy &&
            sellAttempts == 0
          ) {
            sellAttempts++;
...
async function sell(tokenObj, isProfit) {
  try {
    ...
    if (isProfit) {
      balanceString = (
        parseFloat(ethers.utils.formatUnits(bal.toString(), decimals)) *
        (tokenObj.percentOfTokensToSellProfit / 100)
      ).toFixed(decimals);
    } else {
      balanceString = (
        parseFloat(ethers.utils.formatUnits(bal.toString(), decimals)) *
        (tokenObj.percentOfTokensToSellLoss / 100)
      ).toFixed(decimals);
    }
    var roundedBalance = Math.floor(balanceString * 100) / 100;
    const balanceToSell = ethers.utils.parseUnits(
      roundedBalance.toString(),
      decimals
    );
    const sellAmount = await pancakeRouter.getAmountsOut(
      balanceToSell,
      tokenObj.sellPath
    );
...
(async () => {
  ...
  await client.sendMessage("me", {
    message: `Waiting for telegram notification to buy...`,
    schedule: 15 * 1 + Date.now() / 1000,
  });
})();
...
function isStrategy(liquidity, buyTax, sellTax, msg, address) {
  if (config.channel == "CGCMC") {
    if (config.strategy.platform == "BOTH") {
      if (
        liquidity <= config.strategy.maxLiquidity &&
      ...
      if (
        ...
        msg.includes("BNB") &&
        didNotBuy(address) &&
        msg.includes(config.strategy.platform)
      ) {
        return true;
      }
    }
    ...
    if (isStrategy(liquidity, slipBuy, slipSell, msg, address)) {
      token.push({
        ...
        percentOfTokensToSellProfit:
          config.strategy.percentOfTokensToSellProfit,
...
function onNewMessageCoinMarketCapFastestAlerts(message) {
  if (message.peerId.channelId == CoinmarketcapFastestAlertsChannel) {
    console.log(
      "--- NEW TOKEN FOUND FROM COINMARKETCAP FASTEST ALERTS CHANNEL ---"
    );
    let timeStamp = new Date().toLocaleString();
    console.log(timeStamp);
    const msg = message.message.replace(/\n/g, " ").split(" ");
    ...
    if (
      isStrategy(liquidity, slipBuy, slipSell, msg, address) &&
      msg.includes("Insider")
    ) {
      token.push({
        tokenAddress: address,
        didBuy: false,
